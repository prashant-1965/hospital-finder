// Private Hospital Dashboard JavaScript
class PrivateHospitalDashboard {
    constructor() {
        this.apiEndpoint = '/privateHospital';
    ...
    async loadDashboardData() {
        try {
            ...
            this.doctorsData = data.topNPrivateSpecialistDoctor || [];
            this.hospitalsData = data.topNPrivateSpecialistHospitals || [];
    ...
    renderDoctorsList(doctors) {
        ...
        if (!doctors || doctors.length === 0) {
            listElement.innerHTML = this.getEmptyState('No private specialist doctors found', 'fa-user-md');
            return;
        }
            ...
            <div class="doctor-card" data-doctor-id="${index % doctors.length}">
                <div class="card-image-section">
                    <div>
                        ...
                        <div class="coming-soon">Premium</div>
                    </div>
                </div>
    ...
    renderHospitalsList(hospitals) {
        ...
        if (!hospitals || hospitals.length === 0) {
            listElement.innerHTML = this.getEmptyState('No private hospitals found', 'fa-hospital');
            return;
        }
                ...
                <div class="card-content-section">
                    ...
                    <div class="card-info">
                        <div class="card-info-item">
                            ...
                            ${this.escapeHtml(hospital.hospitalType || 'Private Hospital')}
                        </div>
                        <div class="card-info-item">
    ...
    renderDoctorDetailsModal(doctorDetails) {
        ...
        modalContent.innerHTML = `
            <div class="doctor-details-header">
                ...
                <div style="background: linear-gradient(135deg, #667eea, #764ba2); color: white; padding: 0.5rem 1rem; border-radius: 20px; font-size: 0.9rem; margin-top: 1rem; display: inline-block;">
                    <i class="fas fa-crown"></i> Private Practice
                </div>
            </div>
            ...
            <div class="doctor-details-grid">
                ...
                <div class="detail-card">
                    ...
                    <div class="detail-value">
                        <i class="fas fa-stethoscope"></i>
                        ${this.escapeHtml(doctorDetails.doctorType || 'Private Specialist')}
                    </div>
                </div>
            </div>
        `;
    ...
    renderHospitalDetailsModal(hospital) {
        ...
        modalContent.innerHTML = `
            <div class="doctor-details-header">
                ...
                <div class="doctor-specialty">${this.escapeHtml(hospital.hospitalType || 'Private Hospital')}</div>
                <div style="background: linear-gradient(135deg, #667eea, #764ba2); color: white; padding: 0.5rem 1rem; border-radius: 20px; font-size: 0.9rem; margin-top: 1rem; display: inline-block;">
                    <i class="fas fa-crown"></i> Premium Facility
                </div>
            </div>
            ...
            <div class="doctor-details-grid">
                <div class="detail-card">
                    ...
                    <div class="detail-value">
                        <i class="fas fa-building"></i>
                        <span class="age-badge">${this.escapeHtml(hospital.hospitalType || 'Private')}</span>
                    </div>
                </div>
    ...
    async refreshDashboard() {
        try {
            await this.loadDashboardData();
            console.log('Private hospital dashboard refreshed successfully');
        } catch (error) {
...
let privateDashboardInstance = null;

// Initialize dashboard when DOM is loaded
document.addEventListener('DOMContentLoaded', () => {
    privateDashboardInstance = new PrivateHospitalDashboard();
});
...
window.handleGetDetails = function(type, index) {
    ...
    switch(type) {
        case 'doctor':
            if (privateDashboardInstance && privateDashboardInstance.doctorsData[index]) {
                const doctor = privateDashboardInstance.doctorsData[index];
                privateDashboardInstance.showDoctorDetailsModal(doctor.doctorName);
            } else {
                alert('Doctor information not available');
            }
            break;
        case 'hospital':
            if (privateDashboardInstance && privateDashboardInstance.hospitalsData[index]) {
                privateDashboardInstance.showHospitalDetailsModal(index);
            } else {
...
document.addEventListener('visibilitychange', () => {
    if (!document.hidden) {
        setTimeout(() => {
            if (privateDashboardInstance) {
                privateDashboardInstance.refreshDashboard();
            }
        }, 1000);
    }
});